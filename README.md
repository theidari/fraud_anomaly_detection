...
In addition to statistical methods, we can also employ unsupervised learning techniques to detect outliers and anomalies. I utilized the k-means algorithm for this purpose and validated its performance using the Silhouette (silovet) score and the within-cluster sum of squares (WCSS) elbow score. The results of this analysis are presented in the bottom left-hand side of the visualization. The WCSS score suggested an optimal cluster number of 4, while the Silhouette score indicated an optimal cluster number of 3. Ultimately, I decided to proceed with a cluster number of 3, which allows us to identify transactions as safe, medium risk, or high risk. The clustering results are displayed on the right-hand side, specifically for a time rolling period of 3 over October 1997 for check balance in the shortest time rolling period.
